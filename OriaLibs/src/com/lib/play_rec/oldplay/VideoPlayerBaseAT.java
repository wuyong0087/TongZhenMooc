...
public class VideoPlayerBaseAT extends BaseActivity {

	private WakeLock wakeLock = null;
	protected PlayViewGroup viewGroup;
	protected int currPager = 0; // 当前页
	protected PlayView wacomView = null;
	...
	protected void scale(JSONObject commandObj, String type, boolean put)
		...
		if ("Pencil".equals(type) || "Rubber".equals(type)
				|| "Arrow".equals(type) || "Line".equals(type)) { // 画笔和橡皮擦
			JSONObject detailObj = commandObj.getJSONObject("detail");
			if ("Rubber".equals(type)) {
				detailObj.put("thickness", Config.RUBBER_STROKE);
			} else {
				detailObj.put("thickness", detailObj.getInt("thickness"));
			}
			...
			detailObj.put("height",
					calculatePointToInt(detailObj.getDouble("height")));
			JSONObject lastPropertyObj;
			if (commandObj.has("lastProperty")) {
				lastPropertyObj = commandObj.getJSONObject("lastProperty");
			} else {
				lastPropertyObj = detailObj.getJSONObject("lastProperty");
			}
			lastPropertyObj.put("fontSize",
					calculatePointToInt(lastPropertyObj.getInt("fontSize")));
			lastPropertyObj.put("x",
					calculatePointToInt(lastPropertyObj.getDouble("x")));
			lastPropertyObj.put("y",
					calculatePointToInt(lastPropertyObj.getDouble("y")));
			lastPropertyObj.put("width",
					calculatePointToInt(lastPropertyObj.getDouble("width")));
			lastPropertyObj.put("height",
					calculatePointToInt(lastPropertyObj.getDouble("height")));
		} else if ("MoveText".equals(type)) { // 移动文本
			JSONArray dataArray = commandObj.getJSONObject("detail")
					.getJSONArray("data");
			for (int m = 0, n = dataArray.length(); m < n; m++) {
				JSONObject dataObj = dataArray.getJSONObject(m);
	...
	protected void modifyText(JSONObject commandObj) throws JSONException {
		...
		viewGroup.modifyEditText((PlayEditText) groupBean.viewMap
				.get(commandObj.getInt("mid")), lastPropertyObj
				.getString("words"), detailObj.getString("words"),
				lastPropertyObj.getInt("fontSize"), detailObj
						.getInt("fontSize"), HexUtil.hexToColor(lastPropertyObj
						.getString("fontColor")), HexUtil.hexToColor(detailObj
						.getString("fontColor")), lastPropertyObj
						.getInt("width"), detailObj.getInt("width"),
				lastPropertyObj.getInt("height"), detailObj.getInt("height"),
				!pauseRecord);
	}
